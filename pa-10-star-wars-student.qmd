...
> Both codes return the fourth Star Wars title, but one uses the pluck function, while the other uses brackets.
...
```{r}
#The correct line is the second one
sw_films %>% map("title")
...
> First one serves better as a tutorial, the second is the most efficient for an independent project, and the third would serve best in a piped line of code with multiple functions.
...
Now, let's make a plot examining how often hyperdrive ships appear in each episode. Fill in the blanks with the appropriate functions.
...
> Use of hyperdrive is more prevalent in the originals than the prequels.
